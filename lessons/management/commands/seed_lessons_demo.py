...
from lessons.models.course import Course, Category, Lesson
...
class Command(BaseCommand):
    help = "Добавляет много реалистичных уроков в существующие категории (English & Russian)"

    def add_arguments(self, parser):
        parser.add_argument('--reset', action='store_true', help='Удалить демо-данные перед заполнением')
        parser.add_argument('--with-user', action='store_true', help='Создать демо-пользователя')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Заполняем English курс...")
        self._seed_english_course(course_en, create_tg, create_cu)

        self.stdout.write("Заполняем Russian курс...")
        ...
        self.stdout.write(self.style.SUCCESS("✅ Заполнение базы успешно завершено!"))

    # ====================== ENGLISH ======================
    def _seed_english_course(self, course, create_tg, create_cu):
        # Greetings
        cat_greetings, _ = Category.objects.get_or_create(
            course=course, slug='greetings',
            defaults={'title_ky': 'Саламдашуу', 'title_target': 'Greetings',
                      'description_ky': 'Саламдашуу жана коштошуу.', 'icon': 'hand', 'sort_order': 1}
        )
        ...
        greetings_lessons = [
            ...
            ("formal-informal", "Formal vs Informal", "Расмий жана бейрасмий.", 65),
            ("thanks-politeness", "Thank You & Politeness", "Рахмат жана сылык сөздөр.", 60),
        ...
        for i, (slug, title, desc, xp) in enumerate(greetings_lessons, 1):
            lesson, created = Lesson.objects.get_or_create(
                category=cat_greetings, slug=slug,
        ...
        cat_food, _ = Category.objects.get_or_create(
            course=course, slug='food-drinks',
            defaults={'title_ky': 'Тамак-аш жана суусундуктар', 'title_target': 'Food & Drinks',
                      'description_ky': 'Тамак-аш темасы.', 'icon': 'utensils', 'sort_order': 2}
        )

        food_lessons = [
            ("water-basic", "Water and Basic Items", "Суу жана негизги азыктар.", 60),
            ("fruits-vegetables", "Fruits and Vegetables", "Жемиштер жана жашылчалар.", 70),
            ("ordering-cafe", "Ordering in Cafe", "Кафеде заказ кылуу.", 85),
            ("at-market", "At the Market", "Базарда.", 75),
            ("breakfast-meals", "Breakfast & Meals", "Эртең мененки тамак.", 65),
            ("restaurant", "Restaurant Dialogue", "Ресторанда сүйлөшүү.", 80),
        ]

        for i, (slug, title, desc, xp) in enumerate(food_lessons, 1):
            lesson, created = Lesson.objects.get_or_create(
                category=cat_food, slug=slug,
                defaults={'title': title, 'description_ky': desc, 'xp_reward': xp, 'sort_order': i}
            )
            if created:
                self._add_food_lesson(lesson, i, create_tg, create_cu)

    def _add_greetings_lesson(self, lesson, num, create_tg, create_cu):
        if num == 1:
            self._add_basic_hello(lesson, create_tg, create_cu)
        elif num == 2:
            self._add_introducing_yourself(lesson, create_tg, create_cu)
        else:
            # Безопасные шаги с detail_data
            ...
            ContentAuthoringService.create_lesson_step(
                ...
                sort_order=2,
                detail_data={'source_text': 'Салам, кандайсыз?', 'acceptable_answers': ['Hello, how are you?', 'Hi, how are you?']}
            )
    ...
    def _add_basic_hello(self, lesson, create_tg, create_cu):
        ContentAuthoringService.create_lesson_step(
            ...
            prompt_group=create_tg({'ky': '"Салам" англисче кандай?', 'en': 'How do you say "Hello" in English?'}),
            sort_order=1
        )
    ...
    def _add_food_lesson(self, lesson, num, create_tg, create_cu):
        if num == 1:
            ContentAuthoringService.create_lesson_step(
                lesson=lesson, step_type='type_translation',
                prompt='Translate "Нан"',
                sort_order=1,
    ...
    def _seed_russian_course(self, course, create_tg, create_cu):
        cat_greetings, _ = Category.objects.get_or_create(
            course=course, slug='greetings-ru',
            defaults={'title_ky': 'Саламдашуу', 'title_target': 'Приветствия',
                      'description_ky': 'Орусча саламдашуу.', 'icon': 'hand', 'sort_order': 1}
        )

        for i, (slug, title, desc, xp) in enumerate([
            ("basic-hello-ru", "Здравствуй и До свидания", "Негизги саламдашуу.", 50),
            ("introducing-ru", "Представление себя", "Өзүңүздү тааныштыруу.", 70),
        ], 1):
            lesson, created = Lesson.objects.get_or_create(
                category=cat_greetings, slug=slug,
                defaults={'title': title, 'description_ky': desc, 'xp_reward': xp, 'sort_order': i}
    ...
    def _create_demo_user(self, course_en, course_ru=None):
        ...
        for cat in Category.objects.filter(course__in=[course_en, course_ru] if course_ru else [course_en]):
            CategoryProgressService.update_category_progress(user, cat)

        self.stdout.write(self.style.SUCCESS("Демо-пользователь demo_student / demo1234 создан и записан."))
